    ...
    <div class="modal-content px-12 py-20 rounded-lg text-center">
      ...
      <div class="mt-10 px-12 text-lg text-gray-400 text-left">
        <div
          ...
          v-for="(assign, index) in round.specialPowersAssigned"
          :key="index"
        >
          <svg
            fill="currentColor"
          ...
          <div class="ml-2 font-black text-gray-100">
            {{ assign.creature.name }}
          </div>
          <div class="ml-2 text-text-600">
            gain
            <span class="text-yellow-600">{{ assign.power.description }}</span>
          </div>
        </div>
      </div>
    </div>
